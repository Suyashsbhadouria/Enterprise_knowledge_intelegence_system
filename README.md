...
Phase 0 foundation, **Phase 1** Jira Q&A, **Phase 2** Confluence multi-source read, and **Phase 3** GraphRAG: hybrid Neo4j graph + vector retrieval, blocker analysis, LangGraph `understand → retrieve → analyze → answer`.
...
2. Sync: `POST /v1/knowledge/slack/sync` — channel history within `SLACK_SYNC_DAYS` (default 30).
