...
from ekcip_api.services.action_proposals import propose_actions_for_message

from ekcip_api.services.mention_context import prepare_user_message_context

from ekcip_api.services.qa_factory import build_qa_runner

from ekcip_orchestration.actions.llm_detector import merge_action_drafts
...
async def post_message(
    ...
    resolved_text, slack_channel_names, allowed_slack_channels = (

        await prepare_user_message_context(session, settings, payload.content)
    ...
    action_drafts = merge_action_drafts(list(qa_result.action_drafts))
    ...
    note_parts = [f"phase={phase}", f"action_intent={qa_result.action_intent}"]
